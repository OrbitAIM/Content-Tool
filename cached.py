...
def get_cache_file_path(input_data):
    """Generate a unique file path for the given input data."""
    filename = hashlib.md5(str(input_data).encode()).hexdigest()
...
def get_embeddings_with_cache(splits):
    cache_file_path = get_cache_file_path(splits)
    ...
    save_to_cache(cache_file_path, vector)
    
    return vector

# Get embeddings with caching
vector = get_embeddings_with_cache(splits)
